...
public static class QuestionDiffEngine
{
    public static DiffResult Diff(
        IReadOnlyList<RiskLevelQuestion> oldQuestions,
        IReadOnlyList<RiskLevelQuestion> newQuestions)
    {
        ...
        for (int i = 0; i < m; i++)
        {
            ...
            if (j >= 0 && adjScore >= 0.5)
            {
                ...
                bool explanationChanged = TextSimilarity.Score(
                                             oldQ.ExplanationPrompt,
                                             newQ.ExplanationPrompt) < 1.0;
                ...
                if (textChanged || explanationChanged || numberChanged || dvChanged || cfChanged)
    ...
    private static bool IsDvChanged(RiskLevelQuestion old, RiskLevelQuestion @new)
    {
