...
// ── Generic alignment output model ───────────────────────────────────────────
//
// Version-neutral port of CLQ_v01's ClqAlignmentModel. Produced by
// AlignmentEngine.Align<T>. Consumed by version-specific check layers, which turn
// these structures into findings. This layer emits NO severities — the default
// outcome -> severity map lives in the consumer's (overridable) config:
...
public enum CrossYearOutcome
{
    ...
    XrefIdConflict,            // case 2: matcher confidently matches prevA but current.XrefId points to prevB (A!=B) →
                               //   the key and the text disagree about identity. (consumer default: Error)
    ...
    SameXrefIdTextDiverged,    // case 4: current shares an XrefId with a previous question (key asserts SAME identity)
                               ...
                               //   that should have been retired. RAISE ATTENTION. (consumer default: Warning)
...
public sealed record AlignedQuestion<T>(
    T Current,
    // within-year (currentResponse vs emptyTemplate)
    WithinYearJoin WithinYear,
    T? TemplateMatch,                     // non-null IFF WithinYear == JoinedByXrefId
    ...
    T? PreviousMatch,                     // CONFIDENT baseline (deviation/F-integrity); non-null IFF CrossYear == Agree
    T? XrefIdCounterpart,                 // valid-key previous question sharing current's valid XrefId; non-null when
    ...
    T? MatcherCandidate,                  // matcher's textual Hungarian pick (informational; may equal PreviousMatch on Agree); null if none
    double? MatcherBaseScore);            // BASE similarity of MatcherCandidate (NOT adjusted); null if no candidate

public sealed record AlignmentResult<T>(
    IReadOnlyList<AlignedQuestion<T>> Aligned,    // one per currentResponse question, input order preserved
    IReadOnlyList<T> WithinYearRemoved,           // valid-key template questions with no currentResponse counterpart
