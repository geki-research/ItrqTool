...
/// <summary>
/// CLQ_v01-on-core validation (m3). Reads three Control-Level-Questions workbooks (the
/// current response, the empty template, and the previous response), aligns them, runs the
/// version-neutral baseline check catalogue (no extensions — v01 has no answer-stability
/// column), and writes a <see cref="ValidationReport"/> JSON to the <c>report</c> output.
/// Built BESIDE the frozen bespoke v01 stack to prove byte-identity against the m2 goldens;
/// the bespoke stack is untouched.
/// </summary>
/// <remarks>
...
///
/// <see cref="TaskType"/> is the TEMPORARY string <c>ControlLevelQuestionValidation_v01_core</c>,
/// distinct from the bespoke <c>ControlLevelQuestionValidation_v01</c> to avoid the Scrutor/registry
/// collision during this co-existence window. The canonical shipping string is exposed as
/// <see cref="CanonicalTaskType"/>; m4 canonicalizes <see cref="TaskType"/> to it after the bespoke
/// stack retires. The report's <c>taskType</c> field is part of the shipped artifact and is written
/// with <see cref="CanonicalTaskType"/>, not the temporary <see cref="TaskType"/>.
/// </remarks>
public sealed class ControlLevelQuestionValidationV01CoreTask : IWorkflowTask
{
    /// <summary>
    /// The canonical, shipping TaskType string for CLQ_v01 — what the bespoke task carries and what
    /// the report artifact's <c>taskType</c> field records. m4 will make <see cref="TaskType"/> equal this.
    /// </summary>
    public const string CanonicalTaskType = "ControlLevelQuestionValidation_v01";

    private readonly IExcelStructureReader _structureReader;
    private readonly ILogger<ControlLevelQuestionValidationV01CoreTask> _logger;

    public ControlLevelQuestionValidationV01CoreTask(
        IExcelStructureReader structureReader,
        ILogger<ControlLevelQuestionValidationV01CoreTask> logger)
    {
        _structureReader = structureReader;
        _logger = logger;
    }

    public string TaskType => "ControlLevelQuestionValidation_v01_core";
    ...
    public async Task<TaskResult> ExecuteAsync(TaskExecutionContext ctx, CancellationToken ct)
    {
        ...
        try
        {
            ...
            var report = new ValidationReport(config.SheetName, CanonicalTaskType, findings);
