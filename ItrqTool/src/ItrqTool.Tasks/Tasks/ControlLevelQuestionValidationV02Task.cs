...
/// <summary>
/// CLQ_v02 validation. Reads three Control-Level-Questions workbooks (the current
/// response, the empty template, and the previous response), aligns them, runs the
/// check catalogue (baseline + answer-stability extensions), and writes a
...
public sealed class ControlLevelQuestionValidationV02Task : IWorkflowTask
{
    ...
    public string TaskType => "ControlLevelQuestionValidation_v02";
    ...
    public async Task<TaskResult> ExecuteAsync(TaskExecutionContext ctx, CancellationToken ct)
    {
        ...
        try
        {
            ...
            try
            {
                ...
                config = ConfigLoader.Load<ControlLevelQuestionValidationV02Config>(
        ...
        {
            ctx.Logger.LogError(ex, "CLQ v02 validation failed: {Message}", ex.Message);
