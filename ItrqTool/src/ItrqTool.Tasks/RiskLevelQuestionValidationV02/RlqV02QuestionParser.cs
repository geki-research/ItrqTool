...
/// <summary>
/// RLQ_v02 multi-row question parser. Unlike the shared single-row
...
/// <para>
/// The grouping key is the XrefId (column R), which is repeated identically on every row of
...
/// </para>
/// </summary>
public static class RlqV02QuestionParser
{
    public static IReadOnlyList<RlqV02Question> Parse(
        ...
        ICollection<TaskMessage> messages)
    {
        ...
        var questions = new List<RlqV02Question>();
    ...
    private static RlqV02Question BuildQuestion(
        List<ExcelRowStructure> groupRows, string? xref, string sectionName, RlqV02Config config)
    {
        ...
        return new RlqV02Question(
            ...
            HowExplanation:   GetCellText(first, config.HowExplanationColumn));
    }
