...
| Excel I/O | ClosedXML | 0.104.2 | Infrastructure only (Tasks.Tests for fixtures) |
| OPC/OOXML packaging (ClosedXML dependency) | System.IO.Packaging | 10.0.7 | Infrastructure (Tasks.Tests for fixtures) |
| OOXML SDK (LibreOffice load sanitization) | DocumentFormat.OpenXml | 3.1.1 | Infrastructure only |
...
WPF on .NET 10 (`net10.0-windows`), MVVM via CommunityToolkit.Mvvm source generators
(`[ObservableProperty]`, `[RelayCommand]`). ViewModels in
`ItrqTool.Presentation/ViewModels/`, Views (XAML) in `ItrqTool.Presentation/Views/`.

UI-model surrogate records in `src/ItrqTool.Presentation/UIModels/` (authoritative for
signatures) keep Domain types off the bindable surface (non-negotiable rule 5):
`WorkflowListItem`, `WorkflowGroupItem`, `WorkflowLoadFailureItem`, `TaskRowItem` +
`TaskRowStatus`, `TaskParameterItem`, `LogEntry`. None may leak a Domain type onto a
bindable surface.

`AddItrqToolServices(this IServiceCollection, string workflowsDirectoryPath, string
workflowDataRoot)` in `ItrqTool.Presentation` is the **single source of truth for the
production object graph** — never duplicate registrations elsewhere. App.OnStartup and the
integration tests both call it.

The shell pattern (event-based navigation via `ShellViewModel.CurrentViewModel` +
`MainWindow` DataTemplates), `WorkflowRunViewModel` responsibilities (per-row
`TaskRowItem` status derivation, `RunTaskCommand`, the `SelectedTask` configuration
viewer, result-to-log translation that enforces rule 5, `RunButtonLabel` /
`BackCommand` / `OpenWorkingFolderCommand` behaviour), and `WorkflowListViewModel`
load-failure banner behaviour are documented in the `presentation-conventions` skill,
loaded on demand: `.claude/skills/presentation-conventions/SKILL.md`. Force-load it by
name when working on any view model, XAML view, UI-model record, or navigation.
...
Architecture 14, Domain 13, Application 12, Tasks 1170, Infrastructure 154, Integration 161
— **1524 total**.
