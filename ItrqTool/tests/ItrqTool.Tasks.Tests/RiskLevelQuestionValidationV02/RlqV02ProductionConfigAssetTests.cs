...
public sealed class RlqV02ProductionConfigAssetTests
{
    private static string ConfigAssetPath()
    {
        ...
        return Path.Combine(dir.FullName, "configs", "rlq-v02-validation-config.json");
    }
    ...
    public void ProductionConfig_LoadsWithNoValidationErrors()
    {
        ...
        var config = ConfigLoader.Load<RlqV02Config>(json, c => c.Validate());
        ...
        config.MaterialChangeExplanationTriggers.Should().Equal("Yes");
