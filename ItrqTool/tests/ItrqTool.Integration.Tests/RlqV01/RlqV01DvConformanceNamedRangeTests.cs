...
/// <summary>
/// End-to-end tests for the 5b named-range List resolution. Uses the named-range fixture
/// variant (<see cref="RlqV01BaselineFactory.WriteCurrentNamedRange"/> etc.) where the
/// material-change (L) DV is sourced from a workbook-scoped defined name ("MaterialChangeList")
/// that resolves to the "Lists" backing sheet. Mirrors
/// <see cref="RlqV01DvConformanceRangeRefTests"/> in structure and assertion style.
/// </summary>
public sealed class RlqV01DvConformanceNamedRangeTests
{
    private static string TestWorkDir() =>
        Path.Combine(Path.GetTempPath(), "ItrqTool-rlqv01-dv-namedrange", Guid.NewGuid().ToString("N"));
    ...
    [Fact]
    public async Task NamedRangeList_CleanBaseline_EmitsNoConformanceFindings()
    {
        // Template L-DV: List from workbook-scoped name "MaterialChangeList" → Lists!A1:A2
        // ("Yes","No") — named-range resolved via ResolveDefinedNameValues.
        ...
        try
        {
            ...
            RlqV01BaselineFactory.WriteCurrentNamedRange(currentPath);
            RlqV01BaselineFactory.WriteTemplateNamedRange(templatePath);
            RlqV01BaselineFactory.WritePreviousNamedRange(previousPath);
            ...
            report.Findings.Should().NotContain(
                ...
                "clean baseline with named-range List DV must not trigger any DV-conformance finding");
        }
        finally { try { Directory.Delete(dir, recursive: true); } catch (IOException) { } }
    }

    [Fact]
    public async Task NamedRangeList_LOutsideResolvedList_EmitsMaterialChangeNotConformant()
    {
        // Template L-DV: named-range List ("Yes","No") — resolved.
