---
name: clq-validation
description: CLQ_v01 validation task (ControlLevelQuestionValidationV01Task, TaskType "ControlLevelQuestionValidation_v01"). Within-year checks (structure, frozen values/constraints, input validity) and cross-year identity checks against the previous-year response. The 18 CLQ findings from ClqBaselineFindings.All (ids, default severities, ValidationCheck mapping, gating rules), config shape (columns D/E/F/H/I/J/M/N — no K stability column, that is v02's extension), DvPatcher mechanics (one DvRole: answer H), the file-source convention (StaticFileSource now / DynamicFileSource later), the five-node trial workflow topology, and the integration trial (193 real-question trio, 23 seeded findings). Load when working on the CLQ_v01 validation task, its config, the alignment engine, the check catalogue, or the trial scenario.
---
...
downstream `FeedbackChecklistAssembler`. Built on the version-neutral `ItrqTool.Tasks.QuestionnaireValidation`
core via `ClqV01Profile`; `ValidationPipeline.Run<ClqV01Question>` wires read → parse → DV-patch →
align → `ClqBaselineChecks` → serialize.
...
**Config loads** via `ConfigLoader.Load<ClqV01Config>(json, c => c.Validate())`
(the generic loader — NOT a v01-specific loader class). Two-phase: structural errors collected at load
(strict: `UnmappedMemberHandling.Disallow`, bad enum values fail, blank column letters fail, empty
`AllowedAnswers` fail, non-positive `DeviationThreshold` fails — all errors collected before throwing
`ConfigException`); severity-override-key errors inside `ValidationPipeline.Run`.
...
Every finding is identified by a finding-id string from `ClqBaselineFindings.All` (the shared CLQ
catalogue, also used by v02). Default severities may be overridden per finding-id in `SeverityOverrides`.

### Malformed keys (across all three workbooks)

| Finding id | Id | Default | Check | Meaning |
|---|---|---|---|---|
| `structure.xrefid-empty-or-duplicated` | `structure.xrefid-empty-or-duplicated` | **Fatal** | Structure | Identity key in a question row is blank or duplicated within its workbook; the question cannot be reliably matched |
...
| Finding id | Default | Check | Meaning |
|---|---|---|---|
| `structure.question-removed` | Error | Structure | Template question (by identity key) absent from response |
| `structure.question-added` | Error | Structure | Response question absent from template; row not declared by template |
| `structure.question-row-shifted` | Error | Structure | Matched by identity key but on a different row than the template |
| `structure.number-format-unrecognized` | Warning | Structure | Question-number prefix is not a recognised two-level form (e.g. `1.2)`) |

### Within-year: frozen values

| Finding id | Default | Check | Meaning |
|---|---|---|---|
| `static-cell.reference-text-altered` | Warning | FrozenValue | Frozen text (question text, guidance, chapter, section, identity-key text) differs from template; one finding lists all differing fields |
| `static-cell.previous-answer-altered` | Warning | FrozenValue | Col-F injected prior answer differs from prior year's actual answer (col H of previous workbook) |

**Gating — `static-cell.previous-answer-altered`:** runs on **every Agree row**, independently of whether the
...
| Finding id | Default | Check | Meaning |
|---|---|---|---|
| `constraint.answer-validation-rule-changed` | Error | FrozenConstraint | Answer-cell DV rule differs from template (compared via `DvComparer.IsDvChangedFull`) |
...
| Finding id | Default | Check | Meaning |
|---|---|---|---|
| `input-cell.answer-missing` | Error | MissingResponse | Answer cell empty |
| `input-cell.answer-not-in-allowed-set` | **Fatal** | MissingResponse | Answer present but not in `AllowedAnswers`; blocks strengths/weaknesses/deviation checks for this row |
| `input-cell.strengths-missing` | Error | MissingResponse | Answer requires strengths but col-I cell empty |
| `input-cell.weaknesses-missing` | Error | MissingResponse | Answer requires weaknesses but col-J cell empty |
...
Determined by `AlignmentEngine<ClqV01Question>` (core): key + text are compared to find a confident previous
...
| Finding id | Default | Check | Case | Meaning |
|---|---|---|---|---|
| `cross-year.xrefid-conflict` | Error | Structure | Case 2 | Key points to previous row A but text confidently matches previous row B (A≠B); human must verify |
| `cross-year.new-xrefid-resembles-previous` | Warning | Structure | Case 3 | New key absent from previous year, but a textual twin exists; may be a rescope or a mistyped key |
| `cross-year.same-xrefid-text-diverged` | Warning | Structure | Case 4 | Same key as a previous question, but text diverged below match threshold; may be a heavy rewrite or a reused key |
| `cross-year.no-previous-baseline` | Information | Structure | Case 5 | No previous counterpart by key or text; ordinary new/orphan question |
| `cross-year.answer-deviation` | Warning | Deviation | Case 1 (Agree) | `|current − previous| ≥ DeviationThreshold`; displayed as `prevValue → curValue` |
| `cross-year.previous-answer-unusable` | Information | Deviation | Case 1 (Agree) | Confident match exists and current answer is usable, but previous answer is empty or not in allowed set → deviation cannot be evaluated |
...
**Gating — `cross-year.previous-answer-unusable`:** only reached when `answerUsable` is true AND the matched
previous answer is unusable.

## DV patching (DvPatcher)

`ClqV01Profile` declares one `DvRole`: `"answer"` → column H. `DvPatcher` re-reads the
...
Applied to all three workbooks before alignment, via `ValidationPipeline.Run`.
